...
// Define the Project interface with all new fields
interface Project {
  id: string;
  project: string;
...
const ActiveFilters: React.FC<{ filters: ProjectFilters, onRemoveFilter: (type: string, value: string | number) => void, onClearAll: () => void }> = ({ filters, onRemoveFilter, onClearAll }) => {
...
// Project Filter Panel Component
...
const ProjectFilterPanel: React.FC<ProjectFilterPanelProps> = ({ filters, setFilters, onRemoveFilter, onClearAll, allProjects }) => {
  // Extract unique values from projects for dropdown options
  const projectNames = [...new Set(allProjects.map(p => p.project))];
  const memberNames = [...new Set(allProjects.flatMap(p => p.members.map(m => m.name)))];
  const memberEmails = [...new Set(allProjects.flatMap(p => p.members.map(m => `${m.name}@company.com`)))]; // Mock emails
  const domains = [...new Set(allProjects.flatMap(p => p.domain))];
...
export default function Projects() {
  ...
  const handleArchiveConfirm = (newStatus: string) => {
    if (archiveModal.project) {
      setAllProjects((prev) =>
        prev.map((p) =>
          p.id === archiveModal.project!.id ? { ...p, status: newStatus } : p
        )
      );
    }
  };
  ...
  const columns: ColumnDef<Project>[] = [
    ...
    },
    {
      accessorKey: "project",
      header: () => (
        <div className="text-gray-500 text-xs font-medium tracking-wider">Project</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleArchiveToggle = () => {
          const isCurrentlyActive = ["In Progress", "Suspended", "Not Started"].includes(project.status);
          setArchiveModal({
            isOpen: true,
            project: project,
        ...
        const isActive = ["In Progress", "Suspended", "Not Started"].includes(project.status);
	...
	<path fill="currentColor" d="m21.706 5.292l-2.999-2.999A1 1 0 0 0 18 2H6a1 1 0 0 0-.707.293L2.294 5.292A1 1 0 0 0 2 6v13c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V6a1 1 0 0 0-.294-.708M6.414 4h11.172l1 1H5.414zM14 14v3h-4v-3H7l5-5l5 5z"></path>
            ...
            <Button variant="ghost" size="icon" onClick={handleDelete} title="Delete Project" aria-label="Delete Project" className="h-8 w-8">
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setAllProjects(jsonData.projects);
      } catch (error) {
  ...
  const applyFilters = useCallback((projects: Project[]): Project[] => {
    return projects.filter((project) => {
      // Project name filter
      if (filters.projectNames.length > 0 && !filters.projectNames.includes(project.project)) {
        return false;
      }

      // Member name filter
      if (filters.memberNames.length > 0) {
        const projectMemberNames = project.members.map(m => m.name);
        const hasMatchingMember = filters.memberNames.some(filterName => 
          projectMemberNames.includes(filterName)
        );
      ...
      if (filters.memberEmails.length > 0) {
        const projectMemberEmails = project.members.map(m => `${m.name}@company.com`);
        const hasMatchingEmail = filters.memberEmails.some(filterEmail => 
          projectMemberEmails.includes(filterEmail)
        );
      ...
      if (filters.domains.length > 0) {
        const hasMatchingDomain = filters.domains.some(filterDomain => 
          project.domain.includes(filterDomain)
        );
        if (!hasMatchingDomain) return false;
      }

      // Progress filter (min/max)
      if (project.progress < filters.minProgress || project.progress > filters.maxProgress) {
        return false;
      }

      // Budget filter (convert budget strings to numbers for comparison)
      const projectBudget = parseFloat(project.budget.replace(/[$,]/g, ''));
      if (projectBudget < filters.minBudget || projectBudget > filters.maxBudget) {
        return false;
      }

      // Date filters
      if (filters.estimatedStartDate && project.estimatedStartDate !== filters.estimatedStartDate) {
        return false;
      }
      ...
      if (filters.estimatedEndDate && project.estimatedEndDate !== filters.estimatedEndDate) {
        return false;
      }
  ...
  useEffect(() => {
    ...
    const statusFilteredData = activeTab === "active"
      ? allProjects.filter((project) => activeStatuses.includes(project.status))
      : allProjects.filter((project) => archiveStatuses.includes(project.status));

    // Then apply additional filters
    const finalFilteredData = applyFilters(statusFilteredData);

    setData(finalFilteredData);
  }, [allProjects, activeTab, applyFilters]);
  ...
  const handleRemoveFilter = (type: string, value: string | number) => {
    setFilters(prev => ({
      ...prev,
      [type]: Array.isArray(prev[type as keyof ProjectFilters]) 
        ? (prev[type as keyof ProjectFilters] as string[]).filter(v => v !== value)
        : ''
    }));
  };
  ...
  return (
    <div className="min-h-screen pr-5">
      ...
      <div className="flex flex-col gap-8">
        ...
        <PortfolioTabs currentPage="projects" />
...
/>
 ...
 <path fill="currentColor" d="M9.06 9.06a1.5 1.5 0 1 1-2.12-2.12a1.5 1.5 0 0 1 2.12 2.12"></path>
       ...
       <IconPlayerPauseFilled size={40} className="text-[#5477f2] flex-shrink-0" />
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <h2 className="text-lg font-semibold text-gray-900 ml-4">Projects List</h2>
          ...
          <div className="p-6">
            ...
            {/* Content */}
            <DataTable columns={columns} data={data} stickyConfig={stickyConfig} enableLayoutFixed={true} tableId="projects" />
          </div>
        </div>
      </div>
      ...
      <ArchiveStatusModal
        ...
        currentStatus={archiveModal.project?.status || ''}
        itemName={archiveModal.project?.project || ''}
