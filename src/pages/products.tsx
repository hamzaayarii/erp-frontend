...
// Define the Product interface with all new fields
interface Product {
  id: string;
  product: string;
...
const ActiveFilters: React.FC<{ filters: ProductFilters, onRemoveFilter: (type: string, value: string | number) => void, onClearAll: () => void }> = ({ filters, onRemoveFilter, onClearAll }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {filters.productNames.map(name => (
          <div key={name} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-xs flex items-center gap-1">
            Product: {name}
            <button onClick={() => onRemoveFilter('productNames', name)} className="hover:bg-blue-200 rounded">
              <X className="w-3 h-3" />
            </button>
          </div>
        ))}
...
const ProductFilterPanel: React.FC<ProductFilterPanelProps> = ({ filters, setFilters, onRemoveFilter, onClearAll, allProducts }) => {
  // Extract unique values from products for dropdown options
  const productNames = [...new Set(allProducts.map(p => p.product))];
  const memberNames = [...new Set(allProducts.flatMap(p => p.members.map(m => m.name)))];
  const memberEmails = [...new Set(allProducts.flatMap(p => p.members.map(m => `${m.name}@company.com`)))];
  const domains = [...new Set(allProducts.flatMap(p => p.domain))];
  const subProducts = [...new Set(allProducts.map(p => p.subProducts))];
  const subProjects = [...new Set(allProducts.map(p => p.subProjects))];
      ...
      <div className="px-4 space-y-4">
        <div>
          ...
          <Dropdown
            ...
            onChange={(productNames) => setFilters({ ...filters, productNames })}
...
export default function Products() {
  ...
  const getStatusCounts = () => {
    const counts = {
      notStarted: allProducts.filter(p => p.status === 'Not Started').length,
      ...
      finished: allProducts.filter(p => p.status === 'Finished').length,
  ...
  const handleArchiveConfirm = (newStatus: string) => {
    if (archiveModal.product) {
      setAllProducts((prev) =>
        prev.map((p) =>
          p.id === archiveModal.product!.id ? { ...p, status: newStatus } : p
        )
      );
    }
  };
  ...
  const columns: ColumnDef<Product>[] = [
    ...
    },
    {
      accessorKey: "product",
      header: () => (
        <div className="text-gray-500 text-xs font-medium tracking-wider">Product</div>
      ),
      cell: ({ row }) => (
        <div className="text-sm font-bold text-gray-900 px-2">{row.getValue("product")}</div>
      ),
      enableResizing: true,
      size: 180,
      minSize: 150,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleArchiveToggle = () => {
          const isCurrentlyActive = ["In Progress", "Suspended", "Not Started"].includes(product.status);
          setArchiveModal({
            isOpen: true,
            product: product,
        ...
        const isActive = ["In Progress", "Suspended", "Not Started"].includes(product.status);
        ...
        return (
          <div className="flex items-center justify-center gap-1.5">
            ...
            <Button
              ...
              aria-label={isActive ? "Archive Product" : "Unarchive Product"}
              className="h-8 w-8"
            >
              {isActive ? (
    ...
    <path fill="currentColor" d="m21.706 5.292l-2.999-2.999A1 1 0 0 0 18 2H6a1 1 0 0 0-.707.293L2.294 5.292A1 1 0 0 0 2 6v13c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V6a1 1 0 0 0-.294-.708M6.414 4h11.172l1 1H5.414zM14 14v3h-4v-3H7l5-5l5 5z"></path>
            ...
            <Button variant="ghost" size="icon" onClick={handleDelete} title="Delete Product" aria-label="Delete Product" className="h-8 w-8">
  ...
  const applyFilters = useCallback((products: Product[]): Product[] => {
    return products.filter((product) => {
      // Product name filter
      if (filters.productNames.length > 0 && !filters.productNames.includes(product.product)) {
        return false;
      }

      // Sub-products filter
      if (filters.subProducts.length > 0 && !filters.subProducts.includes(product.subProducts)) {
        return false;
      }

      // Sub-projects filter
      if (filters.subProjects.length > 0 && !filters.subProjects.includes(product.subProjects)) {
        return false;
      }

      // Member name filter
      if (filters.memberNames.length > 0) {
        const productMemberNames = product.members.map(m => m.name);
        const hasMatchingMember = filters.memberNames.some(filterName => 
          productMemberNames.includes(filterName)
        );
      ...
      if (filters.memberEmails.length > 0) {
        const productMemberEmails = product.members.map(m => `${m.name}@company.com`);
        const hasMatchingEmail = filters.memberEmails.some(filterEmail => 
          productMemberEmails.includes(filterEmail)
        );
      ...
      if (filters.domains.length > 0) {
        const hasMatchingDomain = filters.domains.some(filterDomain => 
          product.domain.includes(filterDomain)
        );
        if (!hasMatchingDomain) return false;
      }

      // Progress filter (min/max)
      if (product.progress < filters.minProgress || product.progress > filters.maxProgress) {
        return false;
      }

      // Budget filter (convert budget strings to numbers for comparison)
      const productBudget = parseFloat(product.budget.replace(/[$,]/g, ''));
      if (productBudget < filters.minBudget || productBudget > filters.maxBudget) {
        return false;
      }

      // Date filters
      if (filters.estimatedStartDate && product.estimatedStartDate !== filters.estimatedStartDate) {
        return false;
      }
      if (filters.startDate && product.startDate !== filters.startDate) {
        return false;
      }
      if (filters.endDate && product.endDate !== filters.endDate) {
        return false;
      }
      if (filters.estimatedEndDate && product.estimatedEndDate !== filters.estimatedEndDate) {
        return false;
      }
  ...
  useEffect(() => {
    ...
    const statusFilteredData = activeTab === "active"
      ? allProducts.filter((product) => activeStatuses.includes(product.status))
      : allProducts.filter((product) => archiveStatuses.includes(product.status));

    // Then apply additional filters
    const finalFilteredData = applyFilters(statusFilteredData);

    setData(finalFilteredData);
  }, [allProducts, activeTab, applyFilters]);
  ...
  const handleRemoveFilter = (type: string, value: string | number) => {
    setFilters(prev => ({
      ...prev,
      [type]: Array.isArray(prev[type as keyof ProductFilters]) 
        ? (prev[type as keyof ProductFilters] as string[]).filter(v => v !== value)
        : ''
    }));
  };
  ...
  return (
    <div className="min-h-screen pr-5">
      ...
      <div className="flex flex-col gap-8">
        ...
        <PortfolioTabs currentPage="products" />
...
/>
 ...
 <path fill="currentColor" d="M9.06 9.06a1.5 1.5 0 1 1-2.12-2.12a1.5 1.5 0 0 1 2.12 2.12"></path>
       ...
       <IconPlayerPauseFilled size={40} className="text-[#5477f2] flex-shrink-0" />
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <h2 className="text-lg font-semibold text-gray-900 ml-4">Products List</h2>
          ...
          <div className="p-6">
            ...
            {/* Content */}
            <DataTable columns={columns} data={data} stickyConfig={stickyConfig} enableLayoutFixed={true} tableId="products" />
          </div>
        </div>
      </div>
      ...
      <ArchiveStatusModal
        ...
        currentStatus={archiveModal.product?.status || ''}
        itemName={archiveModal.product?.product || ''}
